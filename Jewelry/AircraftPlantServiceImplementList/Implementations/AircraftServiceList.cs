...
namespace AircraftPlantServiceImplementList.Implementations
{
    public class AircraftServiceList : IAircraftService
    {
        ...
        public List<AircraftViewModel> GetList()
        {
            ...
            List<AircraftElementViewModel> aircraftElements = new List<AircraftElementViewModel>();
                for (int j = 0; j < source.AircraftElements.Count; ++j)
                {
                    if (source.AircraftElements[j].AircraftId == source.Aircrafts[i].Id)
                    {
                        ...
                        aircraftElements.Add(new AircraftElementViewModel
                        {
                ...
                result.Add(new AircraftViewModel
                {
                    ...
                    AircraftElements = aircraftElements
                });
            }
            return result;
        }
        public AircraftViewModel GetElement(int id)
        {
            ...
            {
                // требуется дополнительно получить список компонентов для изделия и их количество
            List<AircraftElementViewModel> aircraftElements = new
